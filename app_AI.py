...
def openai_vision_ocr(image_bytes: bytes, document_type: str = None) -> Dict:
    ...
    try:
        ...
        # Prompt spécifique selon le type de document
        if document_type and "FACTURE" in document_type.upper():
            prompt = """
            Analyse cette facture et extrais les informations suivantes au format JSON:
            1. type_document: "FACTURE EN COMPTE"
            2. numero_facture: le numéro de facture
            3. date: la date de la facture
            4. client: le nom du client (S2M, ULYS, DLP/LEADERPRICE)
            5. adresse_livraison: l'adresse de livraison
            6. bon_commande: le numéro de bon de commande
            7. mois: le mois en français
            8. articles: liste d'objets avec "article" et "quantite"
            ...
            Format JSON attendu:
            {
                ...
                "mois": "janvier",
                "articles": [
                    {"article": "Côte de Fianar Rouge 75cl", "quantite": 36},
                    {"article": "Maroparasy Blanc 75cl", "quantite": 24}
                ]
            }
            """
        else:
            prompt = """
            Analyse ce document et identifie s'il s'agit d'une facture ou d'un bon de commande.
            Extrais toutes les informations pertinentes au format JSON.
            
            Si c'est une facture, utilise ce format:
            {
                "type_document": "FACTURE EN COMPTE",
                "numero_facture": "...",
                "date": "...",
            ...
            Si c'est un bon de commande, utilise ce format:
            {
            ...
            Pour les articles, standardise les désignations:
        ...
        if json_match:
            ...
            try:
                ...
                # Standardiser les noms de produits
                if "articles" in data:
                    for article in data["articles"]:
                        article["article"] = standardize_product_name(article.get("article", ""))
...
def find_table_range(worksheet):
    """Trouve la plage de table dans la feuille"""
    try:
        all_data = worksheet.get_all_values()
...
def save_to_google_sheets(document_type: str, data: dict, edited_df: pd.DataFrame, 
...
if uploaded and uploaded != st.session_state.uploaded_file:
    ...
    try:
        ...
        if result:
            ...
            st.session_state.detected_document_type = result.get("type_document", "DOCUMENT INCONNU")
...
if st.session_state.show_results and st.session_state.ocr_result and not st.session_state.processing:
    ...
    with col_nav1:
        if st.button("📄 Scanner un nouveau document", use_container_width=True, type="secondary"):
            st.session_state.uploaded_file = None
            st.session_state.uploaded_image = None
            st.session_state.ocr_result = None
            st.session_state.show_results = False
    ...
    with col_nav2:
        if st.button("🔄 Recommencer l'analyse", use_container_width=True, type="secondary"):
            st.session_state.uploaded_file = None
            st.session_state.uploaded_image = None
            st.session_state.ocr_result = None
            st.session_state.show_results = False
...
<div style="text-align: center; color: {PALETTE['text_medium']}; font-size: 0.9rem; padding: 1.5rem; background: {PALETTE['card_bg']}; border-radius: 12px; margin-top: 2rem; border-top: 1px solid {PALETTE['border']}">
    ...
    <p style="font-size: 0.8rem; margin-top: 0.5rem; opacity: 0.8;">
        ...
        Système OpenAI Vision • Détection automatique des documents
    </p>
</div>
""", unsafe_allow_html=True)
